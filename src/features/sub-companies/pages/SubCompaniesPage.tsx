...
export function SubCompaniesPage() {
  ...
  const columns: Column<SubCompany>[] = [
    {
      key: 'name', header: 'Sub Company', sortable: true,
      accessor: (row) => (
    ...
    { key: 'companyName', header: 'Parent Company', sortable: true, accessor: (r) => <span className="text-sm">{r.companyName}</span> },
    ...
    { key: 'hrCount', header: 'HR Users', accessor: (r) => <span className="font-medium">{r.hrCount}</span>, width: '90px' },
    { key: 'status', header: 'Status', accessor: (r) => <StatusBadge status={r.status} />, width: '100px' },
    { key: 'createdAt', header: 'Created', sortable: true, accessor: (r) => formatDate(r.createdAt), width: '120px' },
    {
